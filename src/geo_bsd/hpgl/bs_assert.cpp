...
namespace bs_assert {
  ...
  asserter::warning ()
  {
    trace_type = TRACE_TYPE_WARNING;
    return this;
  }
  asserter *
  asserter::debug ()
  {
    trace_type = TRACE_TYPE_DEBUG;
    return this;
  }
  asserter *
  asserter::error ()
  {
    trace_type = TRACE_TYPE_ERROR;
    return this;
  }
  asserter *
  asserter::fatal ()
  {
    trace_type = TRACE_TYPE_FATAL;
    return this;
  }
...
//    BOSERR << bs_lock << "Cond: " << pa->cond << "\tFile: " << pa->file << "\tLine: " << pa->line << "\tExpression: " << pa->cond_s << "\nValues: " << pa->var_list << bs_end;
  ...
  trace_wrapper::trace_wrapper (asserter *pa)
  {
    ...
    switch (pa->trace_type)
      {
      case asserter::TRACE_TYPE_WARNING:
        if (!pa->cond)
          {
            trace_warning (pa);
          }
        break;
      case asserter::TRACE_TYPE_DEBUG:
        if (!pa->handle ())
          {
            BREAK_HERE;
          }
        break;
      case asserter::TRACE_TYPE_ERROR:
        if (!pa->cond)
          {
            trace_warning (pa);
#ifdef _DEBUG
        ...
        break;
      case asserter::TRACE_TYPE_FATAL:
        if (!pa->cond)
          {
            trace_warning (pa);
#ifdef _DEBUG
